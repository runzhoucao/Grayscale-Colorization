...
def conv2d(inputs, filters, name, kernel_size=3, strides=2, bnorm=True, activation=None, seed=None):
    """
...
def conv2d_transpose(inputs, filters, name, kernel_size=3, strides=2, bnorm=True, activation=None, seed=None):
    """
...
def to_lab(AB_channels, grayscale):
    # AB_channels batchsize *  256 * 256 * 2            0-1
    # grayscale 0-255

    gray = tf.unstack(grayscale, axis=3)[0]
    a_chan, b_chan = tf.unstack(AB_channels, axis=3)

    img = tf.stack([gray /255.0 - 0.5, a_chan, b_chan], axis=3)
